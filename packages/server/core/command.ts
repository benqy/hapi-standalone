...
export class Dispatcher<R extends Room> {
  ...
  dispatch<T extends Command>(
    command: T,
    payload?: T['payload']
  ): void | Promise<unknown> {
    if (this.stopped) {
      debug(
        `dispatcher is stopped -> ${command.constructor.name} ${
    ...
    if (command.validate && !command.validate(command.payload)) {
      debug(
        `invalid -> ${command.constructor.name} ${
          command.payload ? `(${JSON.stringify(command.payload)})` : ''
        }`
      )
      return
    ...
    if (debug.enabled) {
      debug(
        `execute -> ${command.constructor.name} ${
          command.payload ? `(${JSON.stringify(command.payload)})` : ''
        }`
      )
    }
    ...
    if (hasNextCommands) {
      let lastResult: void | Promise<unknown> = void 0
