...
const BUILD_MODE = (process.env.BUILD_MODE as Environment) || 'production'
console.log(BUILD_MODE, 111)
if (!BUILD_MODE || !['production', 'development'].includes(BUILD_MODE)) {
  throw new Error(
    `[Build] Cannot build with provided environment "${BUILD_MODE}".`
  )
}
...
export async function buildServer() {
  ...
  await build({
    ...
    //         onRebuild: (error, result) => {
    //           console.log(
    //             `[Server] Build finished at ${new Date().toISOString()}`
    //           )
    //         },
    //       },
  })
